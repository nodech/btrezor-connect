...
class EthereumGetPublicKey extends _AbstractMethod.default {
  constructor(message) {
    super(message);
    ...
    if (bundle.length === 1) {
      this.info = (0, _ethereumUtils.getNetworkLabel)('Export #NETWORK public key', bundle[0].network);
    } else {
  ...
  async run() {
    ...
    for (let i = 0; i < this.params.length; i++) {
      ...
      const response = await this.device.getCommands().ethereumGetPublicKey(batch.path, batch.showOnTrezor);
