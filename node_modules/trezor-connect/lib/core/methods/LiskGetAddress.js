...
class LiskGetAddress extends _AbstractMethod.default {
  constructor(message) {
    super(message);
    (0, _defineProperty2.default)(this, "confirmed", false);
    (0, _defineProperty2.default)(this, "progress", 0);
    this.requiredPermissions = ['read'];
    this.firmwareRange = (0, _paramsValidator.getFirmwareRange)(this.name, (0, _CoinInfo.getMiscNetwork)('Lisk'), this.firmwareRange); // create a bundle with only one batch if bundle doesn't exists
    ...
    if (bundle.length === 1) {
      this.info = `Export Lisk address for account #${(0, _pathUtils.fromHardened)(this.params[0].path[2]) + 1}`;
    } else {
  ...
  async run() {
    ...
    for (let i = 0; i < this.params.length; i++) {
      ...
      if (batch.showOnTrezor) {
        const silent = await this.device.getCommands().liskGetAddress(batch.path, false);
      ...
      const response = await this.device.getCommands().liskGetAddress(batch.path, batch.showOnTrezor);
      responses.push({
        path: batch.path,
