...
class CardanoGetPublicKey extends _AbstractMethod.default {
  constructor(message) {
    super(message);
  ...
  async run() {
    ...
    for (let i = 0; i < this.params.length; i++) {
      ...
      const response = await this.device.getCommands().cardanoGetPublicKey(batch.path, batch.showOnTrezor);
      responses.push({
        path: batch.path,
