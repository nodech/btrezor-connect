...
class EosGetPublicKey extends _AbstractMethod.default {
  constructor(message) {
    super(message);
    (0, _defineProperty2.default)(this, "confirmed", false);
    this.requiredPermissions = ['read'];
    this.firmwareRange = (0, _paramsValidator.getFirmwareRange)(this.name, (0, _CoinInfo.getMiscNetwork)('EOS'), this.firmwareRange); // create a bundle with only one batch if bundle doesn't exists
  ...
  async run() {
    ...
    for (let i = 0; i < this.params.length; i++) {
      ...
      const response = await this.device.getCommands().eosGetPublicKey(batch.path, batch.showOnTrezor);
      responses.push({
