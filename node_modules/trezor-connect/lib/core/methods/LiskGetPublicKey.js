...
class LiskGetPublicKey extends _AbstractMethod.default {
  constructor(message) {
    super(message);
    ...
    this.info = 'Export Lisk public key'; // create a bundle with only one batch if bundle doesn't exists
  ...
  async run() {
    ...
    for (let i = 0; i < this.params.length; i++) {
      ...
      const response = await this.device.getCommands().liskGetPublicKey(batch.path, batch.showOnTrezor);
      responses.push({
