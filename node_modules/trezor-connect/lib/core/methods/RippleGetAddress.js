...
class RippleGetAddress extends _AbstractMethod.default {
  constructor(message) {
    super(message);
    ...
    if (bundle.length === 1) {
      this.info = `Export Ripple address for account #${(0, _pathUtils.fromHardened)(this.params[0].path[2]) + 1}`;
    } else {
  ...
  async run() {
    ...
    for (let i = 0; i < this.params.length; i++) {
      ...
      if (batch.showOnTrezor) {
        const silent = await this.device.getCommands().rippleGetAddress(batch.path, false);
      ...
      const response = await this.device.getCommands().rippleGetAddress(batch.path, batch.showOnTrezor);
      responses.push({
