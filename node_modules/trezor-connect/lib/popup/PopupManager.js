...
class PopupManager extends _events.default {
  // Window
  constructor(settings) {
    super();
    (0, _defineProperty2.default)(this, "requestTimeout", 0);
    (0, _defineProperty2.default)(this, "closeInterval", 0);
    (0, _defineProperty2.default)(this, "extensionTabId", 0);
    this.settings = settings;
    this.origin = (0, _networkUtils.getOrigin)(settings.popupSrc);
    this.handleMessage = this.handleMessage.bind(this);
    this.iframeHandshake = (0, _deferred.create)(IFRAME.LOADED);

    if (this.settings.env === 'webextension') {
      this.handleExtensionConnect = this.handleExtensionConnect.bind(this);
      this.handleExtensionMessage = this.handleExtensionMessage.bind(this); // $FlowIssue chrome not declared outside
  ...
  open(lazyLoad) {
    ...
    this.openTimeout = window.setTimeout(() => {
      this.close();
      (0, _showPopupRequest.showPopupRequest)(this.open.bind(this), () => {
        this.emit(POPUP.CLOSED);
      });
    }, POPUP_OPEN_TIMEOUT);
  }
      ...
      chrome.windows.getCurrent(null, currentWindow => {
        ...
        if (currentWindow.type !== 'normal') {
          ...
          chrome.tabs.query({
            ...
            this.extensionTabId = tabs[0].id; // $FlowIssue chrome not declared outside
  ...
  close() {
    ...
    if (this._window) {
      if (this.settings.env === 'webextension') {
        ...
        let _e; // $FlowIssue chrome not declared outside
