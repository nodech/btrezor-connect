...
function udevInstallers(options) {
  const o = options || {};
  const platform = o.platform || preferredPlatform();
  const domain = o.domain || DATA_DOMAIN;
  return UDEV_INSTALLERS.map(i => fillInstallerUrl(i, domain)).map(function (udev) {
    return {
      url: udev.url,
      label: udev.label,
...
function latestVersion(options) {
  ...
  return _fetch(bridgeUrl).then(response => {
...
function installers(options) {
  const o = options || {};
  const version = Promise.resolve(o.version || latestVersion(options));
  return version.then(version => {
    const platform = o.platform || preferredPlatform();
    const domain = o.domain || DATA_DOMAIN;
    return BRIDGE_INSTALLERS.map(i => fillInstallerUrl(i, domain)).map(function (bridge) {
      return {
        version: version,
